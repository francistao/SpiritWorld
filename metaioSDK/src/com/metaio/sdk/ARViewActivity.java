...
//					new FrameLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
			// 下面注释掉的这几行代码是可以实现隐藏水印的，不过看起来镜头被拉近了。
			// mSurfaceView显示的是水印、模型
			DisplayMetrics m = new DisplayMetrics();
			getWindowManager().getDefaultDisplay().getMetrics(m);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams((int)(m.widthPixels * 1.11f), (int)(m.heightPixels * 1.11f));
			
			addContentView(mSurfaceView, params);
			
			mSurfaceView.setX(-m.widthPixels * 0.055f);
			mSurfaceView.setY(0);
...
//					if(msg.what == 5){
//						android.util.Log.i("郑海鹏", "ARViewActivity#handleMessage(): " + "setZOrderMediaOverlay()执行！");
//						android.util.Log.i("郑海鹏", "ARViewActivity#handleMessage(): " + "getParent() :" + mSurfaceView.getParent());
//						mSurfaceView.setZOrderMediaOverlay(true);
//						((ViewGroup)mSurfaceView.getParent()).requestLayout();
